...
st.set_page_config(page_title="Streamlit: Fun Farm",
                   page_icon= ":tractor:")
...
st.title("Fun Farm")
money_col = st.columns([9,1]) #Two columns, the first is 90%, the second is 10% of the width
#money_col[1].write("$"+str(round(st.session_state["user"]["money"], ndigits=2)))

market_tab, field_tab= money_col[0].tabs(["Market", "Field"])

market_columns = market_tab.columns([5,9]) #Two columns, the first is 90%, the second is 10% of the width
market_columns[1].write("Welcome to the Market")
#box = market_tab.container(border=True)
...
with field_tab:
    ...
    #tabs_col, fields_col = st.columns([3,7])
    ...
    with harvest_tab:
        ...
        with first_harvest_plots[0].container(border=True):
            if st.session_state["user"]["plots owned"] >= 0:
                st.text(
                    f"Plot 1\nStatus: {st.session_state["user"]["plots1"]["water"]}\n{st.session_state["user"]["plots1"]["weeds"]}\nCrops: {st.session_state["user"]["plots1"]["plant"]}")
            with st.popover("Harvest"):
                if ((st.session_state["user"]["plots1"]["weeds"] == "no weeds") and \
                    (st.session_state["user"]["plots1"]["plant"] != "empty")) and \
                    (st.session_state["user"]["plots1"]["water"] == "watered"):
        ...
        with first_harvest_plots[1].container(border=True):
            if st.session_state["user"]["plots owned"] > 1:
                st.text(
                    f"Plot 2\nStatus: {st.session_state["user"]["plots2"]["water"]}\n{st.session_state["user"]["plots2"]["weeds"]}\nCrops: {st.session_state["user"]["plots2"]["plant"]}")
                with st.popover("Harvest"):
                    if ((st.session_state["user"]["plots2"]["weeds"] == "no weeds") and \
                        (st.session_state["user"]["plots2"]["plant"] != "empty")) and \
                            (st.session_state["user"]["plots2"]["water"] == "watered"):
        ...
        with second_harvest_plots[0].container(border=True):
            if st.session_state["user"]["plots owned"] > 2:
                st.text(
                    f"Plot 3\nStatus: {st.session_state["user"]["plots3"]["water"]}\n{st.session_state["user"]["plots3"]["weeds"]}\nCrops: {st.session_state["user"]["plots3"]["plant"]}")
                with st.popover("Harvest"):
                    if ((st.session_state["user"]["plots3"]["weeds"] == "no weeds") and \
                        (st.session_state["user"]["plots3"]["plant"] != "empty")) and \
                            (st.session_state["user"]["plots3"]["water"] == "watered"):
        ...
        with second_harvest_plots[1].container(border=True):
            if st.session_state["user"]["plots owned"] > 3:
                st.text(
                    f"Plot 4\nStatus: {st.session_state["user"]["plots4"]["water"]}\n{st.session_state["user"]["plots4"]["weeds"]}\nCrops: {st.session_state["user"]["plots4"]["plant"]}")
                with st.popover("Harvest"):
                    if ((st.session_state["user"]["plots4"]["weeds"] == "no weeds") and \
                        (st.session_state["user"]["plots4"]["plant"] != "empty")) and \
                            (st.session_state["user"]["plots4"]["water"] == "watered"):
            ...
            with st.popover("Select"):
                ...
                if st.session_state["user"]["plots1"]["plant"] == "empty":
                    if st.button("Plant Tomatoes 🍅", key = "pt_plot1"):
                        if st.session_state["user"]["user_seeds"]["tomato seed"] < seeds["tomato seed"]:
                            st.toast("❌ You can't plant that!")
                        else:
                            st.session_state["user"]["user_seeds"]["tomato seed"] -= seeds["tomato seed"]
                            st.session_state["user"]["plots1"]["plant"] = "tomatoes"
                    if st.button("Plant Potatoes 🥔", key="pp_plot1"):
                        if st.session_state["user"]["user_seeds"]["potato seed"] < seeds["potato seed"]:
                            st.toast("❌ You can't plant that!")
                        else:
                            st.session_state["user"]["user_seeds"]["potato seed"] -= seeds["potato seed"]
